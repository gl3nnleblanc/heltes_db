...
- **2PC COMMIT phase has no timeout** — `join_all(commit_futs).await` at `src/coordinator/server.rs:573` has no `tokio::time::timeout` guard; the PREPARE phase correctly wraps with `shard_rpc_timeout` but COMMIT does not; after `send_commit` transitions the tx to COMMITTED (making it visible to Inquirers as committed), a single hung shard stalls the client's `commit()` RPC indefinitely while the coordinator has already durably recorded the decision; wrap the commit `join_all` in `tokio::time::timeout(shard_rpc_timeout, ...)` and surface a retriable error to the client _(3 pts)_
