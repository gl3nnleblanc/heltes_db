...
- **`expire_reads` does not abort evicted transactions** — `expire_reads` silently removes a tx from `read_start_ts` without adding it to `aborted`; a still-alive (slow) client that calls `handle_read` after eviction re-inserts into `read_start_ts` via `or_insert`, but if `compact_versions` ran in the eviction window the MVCC versions the tx needs may already be pruned; the client then receives `NotFound` for a key that had a committed value at its `start_ts`, silently violating SI; fix: add each evicted tx_id to `aborted` in `expire_reads` (mirroring `expire_prepared`) so any subsequent `handle_read` returns `ReadResult::Abort` rather than re-registering with a stale snapshot _(5 pts)_
- **Coordinator clock not periodically re-synced from shards after startup** — `sync_clock_from_shards` runs once at coordinator startup; a coordinator that processes reads but no commits never advances `c_clock` past its startup snapshot; transactions started on it get `start_ts` values that miss all data committed through other coordinators since startup; the Inquire-reply clock-propagation fix (existing TODO) is partial — it only helps when this coordinator happens to inquire about a peer's transaction; add a periodic background task that calls `sync_clock_from_shards` on the coordinator at the same cadence as the shard compact tick _(3 pts)_
