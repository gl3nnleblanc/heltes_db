...
- **Fast-commit coordinator state not set to PREPARING before sending FastCommit RPC** — `begin_fast_commit` leaves `tx.phase = Active` instead of transitioning to `Preparing` as the TLA+ spec requires (`CoordFastCommit` sets `tx_state → PREPARING`); a concurrent `abort_and_notify` can set the phase to Aborted while the shard's `handle_fast_commit` commits the write, leaving coordinator state = Aborted with the write permanently installed in `versions` and visible to future readers; fix: set `tx.phase = TxPhase::Preparing` in `begin_fast_commit` so concurrent aborts are rejected while the RPC is in flight _(5 pts)_
